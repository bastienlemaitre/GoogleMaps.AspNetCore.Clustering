...
using GoogleMaps.AspNetCore.Clustering.Utility;
...
namespace GoogleMaps.AspNetCore.Clustering.Algorithm
{
    ...
    internal class GridCluster : ClusterAlgorithmBase, IGridCluster
    {
        ...
        {
            if (!GmcConfiguration.Get.DoShowGridLinesInGoogleMap) return null; // server disabled it
        ...
        {
            double minDistX = DeltaX / GmcConfiguration.Get.MergeWithin;
            double minDistY = DeltaY / GmcConfiguration.Get.MergeWithin;
...
then the longitudes from 170 to -170 will be clustered together
        ...
        public IList<MapPoint> RunCluster()
        {
            ...
            if (GmcConfiguration.Get.DoMergeGridIfCentroidsAreCloseToEachOther) MergeClustersGrid();

            if (GmcConfiguration.Get.DoUpdateAllCentroidsToNearestContainingPoint) UpdateAllCentroidsToNearestContainingPoint();
